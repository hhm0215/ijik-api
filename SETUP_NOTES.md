...
## 11. timezone: '+09:00'을 풀 설정에 넣은 이유

MySQL 서버의 기본 타임존이 UTC인 경우, `DATETIME` 컬럼에 저장할 때
KST(한국 표준시)와 9시간 차이가 생긴다.

커넥션 풀에 `timezone: '+09:00'`을 설정하면 mysql2가 날짜를 KST 기준으로
변환해서 읽고 쓴다. 서버가 UTC 환경(리눅스 서버 기본값)에 배포될 때도 
날짜 오차가 생기지 않는다.
